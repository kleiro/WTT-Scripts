...
-----
Last Modified: 30th June 2021, 21:33:41
...
function SpawnPVEAircraft(_acType, _spawnCount)
    local _clientInZone = 0
    local _spawnZone = ZONE:FindByName("PVE_Spawn")
    ...
    if _clientInZone > 0 then 
        MESSAGE:New("Spawning ".._spawnCount.." PVE ".._acType):ToAll()
        SPAWN:New("PVE_Red_".._acType)
            :InitGrouping(_spawnCount)
            :Spawn()
    else
        MESSAGE:New("***No aircraft inside the PVE zone. Spawn canceled.***"):ToAll()
    end
end
...
for _k,_v in pairs(WTT.pveAC) do 
    WTT.menus.AC[_v] = MENU_COALITION:New(coalition.side.BLUE, _v, WTT.menus.main)
    for _i=1,4 do
        MENU_COALITION_COMMAND:New(coalition.side.BLUE, tostring(_i), WTT.menus.AC[_v], SpawnPVEAircraft, _v, _i)
    end
end
